...
var selectElement = document.getElementById('job-salary');
selectElement.addEventListener('change', function() {
    var selectedSalary = this.value;
    salary = selectedSalary.split("-");
    console.log(salary);
});
...
function fetchAndRenderJobs(page = 1, pageSize = 10) {
    ...
    fetch(`http://localhost:8000/api/v1/vacancylist?page=${page}&page_size=${pageSize}`, {
        headers: {
...
document.getElementById('searchForm').addEventListener('submit', function(e) {
    ...
    var vacancyLocation = document.getElementById('job-location').value;

    var page = 1; // Текущая страница по умолчанию
    var pageSize = 10; // Размер страницы по умолчанию

    var url = `http://localhost:8000/api/v1/vacancylist/?VacancyName=${encodeURIComponent(vacancyName)}&VacancyGeo=${encodeURIComponent(vacancyLocation)}&min_salary=${encodeURIComponent(salary[0])}&max_salary=${encodeURIComponent(salary[1])}&page=${encodeURIComponent(page)}&page_size=${encodeURIComponent(pageSize)}`;
